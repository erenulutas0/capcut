 ...
 * file name and length when it opened, the refusal message when it did not.
...
const outcome = await Promise.race([
  ...
  page.getByTestId('preview-video').waitFor({ timeout: 60_000 }).then(() => 'opened'),
]);
const text =
  outcome === 'error'
    ? await page.getByTestId('media-error').textContent()
    : await page.getByTestId('source-meta').textContent();
