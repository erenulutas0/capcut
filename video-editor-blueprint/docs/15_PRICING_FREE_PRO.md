...
Politika kimliği: `2026-09-24.v6`. Limitler ürün güvenliği için başlangıç önerisidir; teknik motorların mutlak maksimum kapasitesi değildir. Daha fazla para ödemek desteklenmeyen codec/donanımı destekli yapmaz.
...
| 720p / 1080p, SDR, en çok 30 fps | Desteklenen cihazda dahil | Desteklenen cihazda dahil |
...
**Değişiklik `2026-09-24.v6` (kurucu kararı, 24 Eylül 2026):** Web yerel çıktı kalitesi satırı "30 fps" yerine **"en çok 30 fps"** oldu. Hızlı kesimde (ADR-027: görüntüsü değişmeyen indirmede kaynağın sıkıştırılmış kareleri kopyalanır, yalnızca kesim noktalarından sonraki ilk IDR'a kadarki kareler yeniden kodlanır) kopya, kaynağın **kendi kare hızını (en çok 30 fps) ve kendi bit hızını** korur; bit hızı üst sınırı yoktur, dosya tam kodlamanınkinden büyük olabilir. 24, 25, 29,97 ve 30 fps kaynak ile 30 fps'yi aşmayan değişken hızlı telefon kaydı kendi hızında kopyalanır; "30 fps'yi aşmayan" ölçüsü: bir saniyeye kadar her aralıkta kare sayısı 30 × süre + 1'i, ortalama hız 30,3 fps'yi aşmaz (gerçek kayıtlarda nominal 30 fps dosyalar bir saniyede en çok 31 kare, daha hızlılar 47–61 kare gösterdi). 50/60 fps kaynak, 1440p/4K ve 720p/1080p katmanındaki boyutlardan (1280×720, 720×1280, 720×720, 1920×1080, 1080×1920, 1080×1080) farklı her kaynak eskisi gibi seçilen çözünürlükte 30 fps'ye kodlanır. Dayanak: `docs/adr/ADR-027-fast-cut.md` ve `docs/spikes/2026-09-23-fast-cut.md` ölçümü — Chrome, Edge ve Playwright Chromium'da 60 dakikalık 1080p kesit 437,8 / 373,1 / 804,7 sn yerine 28,1 / 28,6 / 33,7 sn; kopyalanan kareler kaynakla bit bit aynı, kesim noktalarında yeniden kodlanan kareler SSIM ≥ 0,86, ses ≤ 0,06 ms hizalı; kesim hassasiyeti kare düzeyinde. Değişmeyenler: 60 dakika çıktı (diske yazamayan tarayıcıda 5 dakika), 120 dakika girdi, 4 GiB toplam, 5 video kaynağı, 20 parça, 1 müzik, SDR (HDR kaynak yine SDR'ye çevrilerek kodlanır), mobil ve cloud satırları (cloud çıktısı 30 fps kalır), fiyat ve ücretli haklar. Mevcut kullanıcı haklarına etkisi: yalnızca genişleme; önceden indirilebilen her çıktı indirilebilir, kullanıcı yöntem seçmez; hızlı kesimin şartları tutmadığında tam kodlama kendiliğinden çalışır ve sonuç ekranı sebebi söyler. Ölçülmeyen: Safari, Firefox, macOS/iOS ve Android oynatıcıları, Windows oynatıcı uygulamalarının ekranı (yalnızca aynı Media Foundation çözme hattı), başka makineler; ölçümler tek makinede yapıldı (n=1).
